...
showInfo = {
	"Top" : function(heroid) {
		...
		inf.push('<dt>物理攻击</dt><dd><i class="up up1" title="'
				...
				+ '0%"></i></dd>');
		inf.push('<dt>魔法攻击</dt><dd><i class="up up2" title="' + data.info.magic
	...
	"Item" : function(heroid) {
		...
		var data = LOLherojs.champion[heroid].data, itemsHtml = [], ddHtml = [], type = [
				"起始装备", "核心物品", "进攻型物品", "防御型物品" ], blocks = data.blocks;
	...
	"Skill" : function(heroid) {
		...
		g("DATAallytips").innerHTML = '<dt>当你使用' + data.name + '</dt><dd>'
				+ aTmp.join('') + '</dd>';
		aTmp = [];
		for ( var i = 0, l = etips.length; i < l; i++) {
			aTmp.push('<p>' + etips[i] + '</p>')
		}
		g("DATAenemytips").innerHTML = '<dt>敌人使用' + data.name + '</dt><dd>'
				+ aTmp.join('') + '</dd>';
	}
};
//运行
